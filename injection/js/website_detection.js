(function () {
  ...
  if (detection_header) {
    ...
    detection_header.addEventListener('mousedown', function (e) {
      ...
      function handlerMouseMove(e) {
        website_detection.style.left = Math.max(0, Math.min(
          document.documentElement.clientWidth - website_detection.clientWidth,
          origin[0] + (e.screenX - startPos[0]))
        ) + 'px'
        website_detection.style.top = Math.max(0, Math.min(
          document.documentElement.clientHeight - website_detection.clientHeight,
  ...
  var detection_close = website_detection.querySelector('.panel_header .btn_close')
